...
def parse_commit_tree(json_data: dict, commit_sha: str, repo_name: str):
    """Parsea el AST y extrae metadatos igual que 5a, inyectando métricas del dominio."""
    try:
        ...
        for label in cat.children:
            ...
            for rule in label.children:
                ...
                label_node["children"].append({
                    ...
                    "metrics": {
                        "len": len(text),
                        "sym": getattr(rule, 'special_chars_count', 0)
                    },
...
<head>
    ...
    <style>
        ...
        .legend-panel {{
            ...
            border: 1px solid rgba(226, 232, 240, 0.8); border-radius: 8px;
            padding: 12px 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);
            z-index: 100; font-size: 0.8rem; color: #475569; pointer-events: none;
        }}
        .legend-panel h4 {{ margin: 0 0 8px 0; font-size: 0.85rem; color: #0f172a; text-transform: uppercase; letter-spacing: 0.5px; }}
        .legend-grid {{ display: grid; grid-template-columns: auto auto; gap: 8px 16px; }}
        .legend-item {{ display: flex; align-items: center; gap: 6px; }}
        .legend-badge {{
            font-size: 0.65rem; font-weight: bold; padding: 2px 4px;
            border-radius: 3px; font-family: monospace; border: 1px solid;
        }}
        .lb-lbl {{ background: #dbeafe; color: #1e40af; border-color: #bfdbfe; }}
        .lb-rul {{ background: #d1fae5; color: #065f46; border-color: #a7f3d0; }}
        .lb-cod {{ background: #f3e8ff; color: #6b21a8; border-color: #e9d5ff; }}
        .lb-fre {{ background: #ffedd5; color: #9a3412; border-color: #fed7aa; }}
        .lb-len {{ background: #f1f5f9; color: #475569; border-color: #cbd5e1; }}
        .lb-sym {{ background: #fef2f2; color: #991b1b; border-color: #fecaca; }}
...
<body>
    ...
    <div class="legend-panel">
        ...
        <div class="legend-grid">
            ...
            <div class="legend-item"><span class="legend-badge lb-fre">FRE</span> Readability Score</div>
            <div class="legend-item"><span class="legend-badge lb-len">LEN</span> Text Length</div>
            <div class="legend-item"><span class="legend-badge lb-sym">SYM</span> Special Symbols</div>
        </div>
    </div>
    ...
    <script>
        ...
        // [MODIFICADO] Aumentado el espaciado vertical de 60 a 85 para evitar superposiciones con los badges del nodo inferior
        ...
        function updateGraph(index) {{
            ...
            // =========================================================================
            // INYECCIÓN VISUAL DE MÉTRICAS EXTERNAS (Píldoras Flotantes)
            // =========================================================================
            nodeUpdate.each(function(d) {{
                const el = d3.select(this);
                // Limpiar métricas anteriores para redibujar
                el.selectAll(".metric-badges-container").remove();

                if (!d.data.metrics || d.data.group === 'root') return;

                let badges = [];
                // Configuramos los colores estilo "GitHub Labels"
                ...
                // [MODIFICADO] translate(0, -25): X=0 (alineado izq), Y=-25 (acerca la píldora al borde superior del rect que inicia en -18)
                const badgeGroup = el.append("g")
                    .attr("class", "metric-badges-container")
                    .attr("transform", `translate(0, -25)`); 

                let currentX = 0;
                badges.forEach(b => {{
                    const bg = badgeGroup.append("g").attr("transform", `translate(${{currentX}}, 0)`);
                    const textStr = `${{b.label}}: ${{b.val}}`;
                    
                    const t = bg.append("text")
                        .attr("font-size", "8px")
                        .attr("font-family", "system-ui, -apple-system, sans-serif")
                        .attr("font-weight", "700")
                        .attr("fill", isGhost ? '#94a3b8' : b.color)
                        .text(textStr);
                    ...
                    bg.insert("rect", "text")
                        ...
                        .attr("y", -10)
                        .attr("rx", 3)
                        .attr("fill", isGhost ? '#f8fafc' : b.bg)
                        .attr("stroke", isGhost ? '#e2e8f0' : b.border)
                        .attr("stroke-width", 1);
                        
                    t.attr("x", 4);
                    
                    currentX += w + 4; // Gap horizontal entre píldoras
                }});
            }});
            // =========================================================================
...
def main():
    parser = argparse.ArgumentParser(description="Generate Interactive Metrics Diff Evolution Tree.")
    ...
    generate_html(evolution_data, out_dir / f"{args.repo_name}_5c_diff_metrics_tree.html")
