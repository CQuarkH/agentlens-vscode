...
def generate_html(md_content, doc, output_path, md_filename):
    repo_name = escape(doc.repo_name)
    md_source = escape(md_filename)
...
<body>
    <div class="header-bar">
        <h1 class="header-title">{repo_name} - Evolution AST View</h1>
    </div>
...
def main():
    parser = argparse.ArgumentParser(description="Generate Evolution Tree HTML Graph.")
    ...
    # LÓGICA EVOLUTIVA: Extraemos el repo y el SHA directamente de la ruta del JSON
    repo_name_folder = json_path.parent.name
    commit_sha = json_path.stem
    md_filename = f"{commit_sha}.md"
    
    # Buscamos en la nueva carpeta caché
    md_path = root_dir / "dataset" / "evolution_exp" / "agents_cache" / repo_name_folder / md_filename
    ...
    if md_path.exists():
        try:
            ...
            md_content = f"Error reading markdown file: {e}"
    else:
        logger.warning(f"Raw markdown no encontrado en caché: {md_path}")
        md_content = f"⚠️ RAW MARKDOWN NOT FOUND FOR THIS COMMIT.\n\nExpected path:\n{md_path}\n\nThe AST Tree was generated successfully using only the JSON data."

    output_dir = root_dir / "dataset" / "visualizations"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    output_path = output_dir / f"{commit_sha}_tree_viz.html"
    generate_html(md_content, doc, output_path, md_filename)
